...
from det3d.apis import train_semi_model
...
def modify_file_client_args(cfg):
    ...
    for idx, item in enumerate(cfg.train_pipeline):
        if item['type'] == 'LoadMultiViewImageFromFiles' or \
            ...
            item['type'] == 'LoadPointsFromFile':
            item['file_client_args'] = file_client_args
            # break
            cfg.train_pipeline[idx] = item
    if cfg.get("train_semi_pipeline", None) is not None:
        for idx, item in enumerate(cfg.train_semi_pipeline):
            if item['type'] == 'LoadMultiViewImageFromFiles' or \
                item['type'] == 'CustomLoadMultiViewImageFromFiles' or \
                item['type'] == 'LoadImageFromFileMono3D' or \
                item['type'] == 'LoadImageFromFile' or \
                ...
                item['file_client_args'] = file_client_args
                # break
                cfg.train_semi_pipeline[idx] = item
    ...
    if "train_semi_pipeline" in cfg.data.train:
        cfg.data.train.train_semi_pipeline = cfg.train_semi_pipeline

    if 'dataset' in cfg.data.train and 'pipeline' in cfg.data.train.dataset:
        cfg.data.train.dataset['pipeline'] = cfg.train_pipeline
    if 'dataset' in cfg.data.train and "train_semi_pipeline" in cfg.data.train.dataset:
        cfg.data.train.dataset.train_semi_pipeline = cfg.train_semi_pipeline
...
def main():
    ...
    if args.work_dir is not None:
        ...
        project_name = osp.join(osp.splitext(osp.basename(args.config))[0])
    ...
    work_dir = osp.join(work_dir, timestamp)
    project_name = osp.join(project_name, timestamp)
    ...
    data = datasets[0].__getitem__(0)
    semi_train = cfg.data.get("semi_train", None)
    if semi_train is not None:
        semi_datasets = [build_dataset(cfg.data.semi_train)]
    else:
        semi_datasets = None
