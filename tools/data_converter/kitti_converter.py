...
from .kitti_data_utils import WaymoInfoGatherer, get_kitti_image_info, get_kitti_raw_image_info
...
def create_kitti_raw_info_file(data_path,
                               pkl_prefix='kitti',
                               save_path=None,
                               relative_path=True):
    imageset_folder = Path(data_path) / 'ImageSets'

    with open(osp.join(imageset_folder, 'raw_clean.txt')) as f:
        raw_clean_ids = f.read().split("\n")
    with open(osp.join(imageset_folder, 'raw.txt')) as f:
        raw_ids = f.read().split("\n")

    print('Generate raw kitti info. This may takes several minutes')
    ...
    kitti_infos_raw_clean = get_kitti_raw_image_info(
        data_path,
        velodyne=True,
        calib=True,
        image_ids=raw_clean_ids,
        relative_path=relative_path)
    filename = save_path / f'{pkl_prefix}_infos_raw_clean.pkl'
    print(f'Kitti info eigen vlean file is saved to {filename}')
    mmcv.dump(kitti_infos_raw_clean, filename)
    '''

    kitti_infos_raw = get_kitti_raw_image_info(
        data_path,
        velodyne=True,
        calib=True,
        image_ids=raw_ids,
        relative_path=relative_path)
    filename = save_path / f'{pkl_prefix}_infos_raw.pkl'
    print(f'Kitti info eigen vlean file is saved to {filename}')
    mmcv.dump(kitti_infos_raw, filename)
    '''
