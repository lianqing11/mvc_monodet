...
def get_raw_image_path(idx,
                      prefix,
                      info_type = "kitti_raw",
                      exist_check=True,):
    # assert input the raw path
    img_idx_str = idx
...
def get_raw_velodyne_path(idx,
                          prefix,
                          info_type="kitti_raw",
                          exist_check=True):
    img_idx_str = idx
    img_idx_str = img_idx_str.replace("image_02", "velodyne_points")
    img_idx_str = img_idx_str.replace("png", "bin")
    file_path = Path(info_type) / img_idx_str
    prefix = Path(prefix)
    if exist_check and not (prefix / file_path).exists():
        raise ValueError('file not exist: {}'.format(file_path))
...
def get_raw_calib_path(idx,
                        prefix,
                        info_type="kitti_raw",
                        exist_check=True):
    calib_keys = ['calib_cam_to_cam',
                 'calib_imu_to_velo',
                 'calib_velo_to_cam']
    calib_idx_str = idx
    calib_idx_str = calib_idx_str.split("/")[0]
    calib_dict = {}
    prefix = Path(prefix)
    info_type = Path(info_type)
    for key in calib_keys:
        calib_idx = calib_idx_str + "/" + key + ".txt"
        calib_dict[key] = info_type / Path(calib_idx)
        if exist_check and not (prefix / calib_dict[key]).exists():
            raise ValueError('file not exist: {}'.format(calib_dict[key]))
    return calib_dict
...
def get_kitti_raw_image_info(path,
                         velodyne=True,
                         calib=True,
                         image_ids=7481,
                         extend_matrix=True,
    ...
    for idx in tqdm(image_ids):
        ...
        image_info = {'image_idx': idx}
        if velodyne:
            pc_info['velodyne_path'] = get_raw_velodyne_path(idx, path).split("/")
            pc_info['velodyne_path'] = "/".join(pc_info['velodyne_path'][2:])
        image_info['image_path'] = get_raw_image_path(idx, path)
        image_info['image_path'] = image_info['image_path'].split("/")
        temp_path = [image_info['image_path'][0]]
        temp_path.extend(image_info['image_path'][2:])
        image_info['image_path'] = "/".join(temp_path)
        if with_imageshape:
            img_path = image_info['image_path']
            if relative_path:
                img_path = str(root_path / img_path)
        ...
        info['point_cloud'] = pc_info
        if calib:
            calib_path_list = get_raw_calib_path(
                idx, path)
            with open(root_path/calib_path_list['calib_cam_to_cam'], 'r') as f:
                lines = f.readlines()

            P0 = np.array([float(info) for info in lines[9].split(' ')[1:13]
                           ]).reshape([3, 4])
            P1 = np.array([float(info) for info in lines[17].split(' ')[1:13]
                           ]).reshape([3, 4])
            P2 = np.array([float(info) for info in lines[25].split(' ')[1:13]
                           ]).reshape([3, 4])
            P3 = np.array([float(info) for info in lines[33].split(' ')[1:13]
                           ]).reshape([3, 4])
            if extend_matrix:
                P0 = _extend_matrix(P0)
                P1 = _extend_matrix(P1)
                P2 = _extend_matrix(P2)
                P3 = _extend_matrix(P3)
            # with open(calib_path_list['calib_velo'])
            R0_rect = np.array([
            ...
            with open(root_path/calib_path_list['calib_velo_to_cam'], 'r') as f:
                lines = f.readlines()

            Tr_velo_to_cam_r = np.array([
                float(info) for info in lines[1].split(' ')[1:13]
            ]).reshape([3, 3])
            Tr_velo_to_cam_t = np.array([
                float(info) for info in lines[2].split(' ')[1:4]
            ]).reshape([3, 1])
            Tr_velo_to_cam = np.concatenate([Tr_velo_to_cam_r, Tr_velo_to_cam_t], axis=1)


            with open(root_path/calib_path_list['calib_imu_to_velo'], 'r') as f:
                lines = f.readlines()

            Tr_imu_to_velo_r = np.array([
                float(info) for info in lines[1].split(' ')[1:13]
            ]).reshape([3, 3])
            Tr_imu_to_velo_t = np.array([
                float(info) for info in lines[2].split(' ')[1:4]
            ]).reshape([3, 1])
            Tr_imu_to_velo = np.concatenate([Tr_imu_to_velo_r, Tr_imu_to_velo_t], axis=1)
