...
from mmdet3d.datasets.pipelines import RandomFlip3D, LoadMultiViewImageFromFiles
...
# from mmdet3d.d
from det3d.core.bbox.util import projected_gravity_center

@PIPELINES.register_module()
class LoadMultiviewSegMaskFromFile(object):
    """Load multiview seg mask from file"""

    def __init__(self, preprocess_mask=False):
        self.pre_process_mask = preprocess_mask

    def __call__(self, results):
        """Call function to load multi-view seg mask from files"""
        seg_mask_list = []
        seg_box_list = []
        for seg_mask_path in results['seg_mask_filename']:
            if osp.exists(seg_mask_path):
                seg_data = np.load(
                    seg_mask_path)
                seg_mask, seg_box = seg_data["pred_mask"], seg_data["pred_boxes"]
                seg_mask_list.append(seg_mask)
                seg_box_list.append(seg_box)
            else:
                continue
        if self.pre_process_mask:
            assert len(seg_mask_list) == 1

        results["gt_multiview_semantic_seg"] = seg_mask_list
        return results
...
class CustomRandomFlip3Dv2(object):
    ...
    def flip_img(self, results, direction='horizontal'):
        ...
        if 'gt_multiview_semantic_seg' in results:
            results['gt_multiview_semantic_seg'] = [np.flip(seg_mask, axis=2) for seg_mask in \
    ...
    def flip_bbox_cam_params(self, results):
        ...
        # print(lidar2cam_idx, cam2img_idx)
        box_cam = results['gt_bboxes_3d'].convert_to(Box3DMode.CAM, rt_mat=lidar2cam_idx)
        # centers = projected_gravity_center(box_cam, torch.tensor(cam2img_idx))
        # print(centers)

        box_type = type(results['gt_bboxes_3d'])
        ...
        if box_type == CameraInstance3DBoxes:
            results['gt_bboxes_3d'] = box_cam
        else:
            box_lidar = box_cam.convert_to(Box3DMode.LIDAR, rt_mat=np.linalg.inv(lidar2cam_idx))
            results['gt_bboxes_3d'] = box_lidar
        # centers = projected_gravity_center(box_cam, torch.tensor(cam2img_idx))
        # print(centers, box_cam)
        return results


@PIPELINES.register_module()
class CustomRandomFlip3Dv3(object):
    def __init__(self, flip_ratio=0.5):
        self.flip_ratio=flip_ratio
    ...
    def flip_bbox_cam_params(self, results):
        ...
        # box_type = results['gt_bboxes_3d']

        w = results['img_shape'][1]
        for idx in range(len(results['lidar2cam'])):
            lidar2cam = results['lidar2cam'][idx]
            cam2img = results['cam2img'][idx]
            lidar2img = results['lidar2img'][idx]
            cam2img[0,0]*= -1
            cam2img[0,2] = w - cam2img[0,2]
            cam2img[0,3] *= -1
            lidar2img = lidar2cam @ cam2img
            # results['lidar2cam'][idx] = lidar2cam
