...
class OCSORTTracker(SortTracker):
    ...
    def vel_direction(self, bbox1, bbox2):
        ...
        if bbox1.sum() < 0 or bbox2.sum() < 0:
            if self.overlap_mode == "align_bev":
                return torch.tensor((-1, -1))
            elif self.overlap_mode == "bev":
                return torch.tensor((-1, -1, -1))
            elif self.overlap_mode == "3d":
                return torch.tensor((-1, -1, -1))
        
        speed = bbox2[:self.loc_var] - bbox1[:self.loc_var]

        norm = torch.sqrt((speed**2).sum() + 1e-6)
        return speed / norm
        

    def vel_direction_batch(self, bboxes1, bboxes2):
        """Estimate the direction vector given two batches of boxes."""
        loc1 = bboxes1[:, :self.num_loc_var]
        loc2 = bboxes2[:, :self.num_loc_var]
        ...
        #                   dim=-1)
        speed = loc2 - loc1
        norm = torch.sqrt((speed**2).sum(1, keepdim=True))
        speed = speed / norm
        return speed
    ...
    def ocm_assign_ids(self,
        ...
        Args:
            ...
            det_bboxes (Tensor): of shape (N, 4) or lidar bboxes
        ...
        """
        # get track_bboxes
        track_bboxes = np.zeros((0, self.num_bbox_var))


        for id in ids:
            track_bboxes = np.concatenate(
                (track_bboxes, self.tracks[id].mean[:self.num_bbox_var][None]), axis=0)

        track_bboxes = torch.from_numpy(track_bboxes).to(det_bboxes)
        # track_bboxes = bbox_cxcyah_to_xyxy(track_bboxes)

        # compute distance
        ious = self.iou_func(track_bboxes, det_bboxes)
        ...
        if len(ids) > 0 and len(det_bboxes) > 0:
            ...
            vel_to_match = self.vel_direction_batch(k_step_observations[:, :self.num_bbox_var],
                                                    det_bboxes[:, :self.num_bbox_var])
    ...
    def track(self,
        ...
        if self.empty or bboxes.size(0) == 0:
            valid_inds = confidence[:, -1] > self.init_track_thr
            bboxes = bboxes[valid_inds]
            labels = labels[valid_inds]
            confidence = confidence[valid_inds]
            ...
            det_inds = confidence[:, -1] > self.obj_score_thr
            det_bboxes = bboxes[det_inds]
            det_labels = labels[det_inds]
            det_confidences = confidence[det_inds]
