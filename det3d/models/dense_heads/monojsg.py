...
from .nocs_head import unnormalized_coordinate

from mmcv import ops


from .bev_object_head import _mlp_module
from det3d.models.necks.imvoxel_neck import BasicBlock3dV2
...
class MonoJSGHead(TwoStageHead):
    ...
    def __init__(self,
        ...
        self.loss_bbox = build_loss(loss_reg)
    ...
    def loss(self, refine_preds, gt, bbox_list):
        ...
        losses = {}
        if reg_mask.sum() > 0:
            losses["loss_2stage_depth"] = F.l1_loss(refine_preds["reg_depth"], gt["reg"][:, 2:3].detach())

        # get the corresponding depth
        # summarize the depth
        # compare with gt depth (l1 loss)
        # return loss
        # output
        # losses["loss_reg"] = xxx
        return losses
    ...
    def select_features(self, bbox_list, features, img_metas, preds):
        ...
        if normalize_nocs:
            ...
            # gravity center
            roi_nocs = unnormalized_coordinate(roi_nocs, pred_dim, pred_yaw).detach()
...
class RefineCostNetV1(nn.Module):
    ...
    def __init__(self, input_planes, feat_planes):
        ...
        self.feature_extractor3d = nn.Sequential(
                ...
                BasicBlock3dV2(feat_planes, feat_planes),)
